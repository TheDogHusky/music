...
export function useButtonVariants(props: ButtonProps | ButtonLinkProps | NuxtButtonLinkProps): string {
    const themeVariants: { [key: string]: string } = {
        ...
        icon: "rounded-none border-none text-primary hover:text-slate-300 hover:border-slate-300 hover:scale-105 h-auto w-auto hover:bg-transparent text-2xl p-2 flex flex-row items-center justify-center",
    };
    ...
    return twMerge(
        "rounded-none duration-200 border-2 border-gray-300 outline-none focus:ring-3 text-base text-gray-300 font-semibold focus:ring-gray-300 focus:ring-opacity-50 bg-transparent hover:bg-gray-300 hover:text-neutral-950 flex flex-row items-center justify-center gap-4",
        sizeVariants[props.size || "md"],
        themeVariants[props.variant || ""],
        specialVariants[props.special || ""],
