...
export function useButtonVariants(props: ButtonProps | ButtonLinkProps | NuxtButtonLinkProps): string {
    const themeVariants: {[key: string]: string} = {
        ...
        icon: "rounded-none border-s-0 border-t-0 border-e-0 text-gray-300 border-b-2 pb-2 border-dashed border-secondary hover:text-gray-100 hover:border-gray-300 hover:scale-105 h-10 w-10 hover:bg-transparent text-2xl p-2 flex flex-row items-center justify-center",
    };
